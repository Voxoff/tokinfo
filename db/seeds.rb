...
data["Data"].each_with_index do |(key,value), i|
  ...
  if i < 5
    ...
    business = Business.create!(user_id: user.id, name: Faker::Company.name, address: Faker::Address.city, url: Faker::Internet.url, email: Faker::Internet.email, remote_photo_url: "https://source.unsplash.com/random/400x400", description: "tWhat if you want to render a template from an entirely different controller from the one that contains the action code? You can also do that with render,")
