...
businesses = ["Microsoft", "Facebook", "Amazon", "Google", "Victoria's Secret"]
...
# admin account for easy access
  ...
  admin_business = Business.create!(user_id: admin.id, name: "Le Wagon", address: "admin street", url: "admin.com", email: "admin@admin.com", remote_photo_url: "https://source.unsplash.com/random/400x400")
...
data["Data"].each_with_index do |(key,value), i|
  ...
  if i < 5
    ...
    business = Business.create!(user_id: user.id, name: businesses[i], address: Faker::Address.city, url: Faker::Internet.url, email: Faker::Internet.email, remote_photo_url: "https://source.unsplash.com/random/400x400&sig=#{i}", description: "Businesses that have compelling ethoses end up winning clients even when their prices are not the best in the market or their products or services might not be the most innovative available. Their ethoses may capture the attention of their niches, and their clients develop an emotional attachment.")
